...
public class ArrayList {
    ...
    of pivot */
	private int partition(WordLinkedList arr[], int low, int high) { //Algorithm supplied by geeksforgeeks
		String pivot = arr[high].getHead().getOriginalWord();
		int i = (low-1); // index of smaller element
		for (int j=low; j<high; j++)
		{
			// If current element is smaller than or equal to pivot
			if (arr[j].getHead().getOriginalWord().compareToIgnoreCase(pivot) <= 0) {
             i++;
             // swap arr[i] and arr[j]
             WordLinkedList temp = arr[i];
             arr[i] = arr[j];
             arr[j] = temp;
			}
		}
		// swap arr[i+1] and arr[high] (or pivot)
		WordLinkedList temp = arr[i+1];
		arr[i+1] = arr[high];
		arr[high] = temp;
		return i+1;
		}
	 ...
	 * low  --> Starting index, 
	 * high  --> Ending index */
	private void sort(WordLinkedList arr[], int low, int high) { 
		if (low < high) {
